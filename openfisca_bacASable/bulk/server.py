...
class Cache(object):
  ...
  def unload(self):
    ...
    logging.info('unload cache (' + str(len(self.data)) + ')')
...
def getDailySituations(limit):
  ...
  today = datetime.datetime.combine(datetime.date.today() - datetime.timedelta(1), datetime.datetime.min.time())
...
def processSituations(cursor, remote=False, cache=None):
  ...
  for s in cursor:
    ...
    try:
      if cache:
        ...
        situation = fetchSituation(sid, s['token'], remote)
    except KeyError:
        situation = fetchSituation(sid, s['token'], remote)
    finally:
      cache[sid] = situation
...
def fetchSituation(sid, secret, remote=False):
  ...
  r = requests.get(url, cookies={"situation_" + sid : secret})
