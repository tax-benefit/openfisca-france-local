...
def main():
    ...
    remoteSituations = server.processSituations(s, remote=False, cache=cache)
...
#        'cheque_energie': allMonths,
    ...
    for calcul, periods in calculs.iteritems():
        for period in periods:
            injected = simulation_actuelle.get_array(calcul, period)
            simulation_actuelle.get_holder(calcul).delete_arrays(period)
            results['known_' + calcul] = injected

    log.info('calculate')
    for calcul, periods in calculs.iteritems():
        for period in periods:
            values = simulation_actuelle.calculate(calcul, period)
            results[calcul] = values

    log.info('save')
    results.to_csv('file2.csv', index=False, decimal=",", sep=";")
    log.info('end')
